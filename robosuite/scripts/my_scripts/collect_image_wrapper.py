...
class DataCollectionImageWrapper(Wrapper):
    def __init__(self, env, directory, collect_freq=1, flush_freq=100):
        ...
        self.states = []
        self.gripper_states = [] # gripper states
        self.image_obs = [] # frontal camera image observation
    ...
    def _flush(self):
        ...
        np.savez(
            ...
            gripper_state=np.array(self.gripper_states),
            image_obs=np.array(self.image_obs),
    ...
    def step(self, action):
        ...
        # cv2.imwrite('test_obs.png',env_state['agentview_image'][::-1,:,::-1])
        ...
        if self.t % self.collect_freq == 0:
            ...
            self.gripper_states.append(np.concatenate([env_state['robot0_eef_pos'], env_state['robot0_eef_quat']])) 
            # collect image observation
            self.image_obs.append(env_state['agentview_image'])
